...
#define ETHOS_CACHE_BUF_ATTRIBUTES  __attribute__((section("ethos_cache"), aligned(ETHOS_CACHE_BUF_ALIGNMENT)))
#endif
#endif
...
/* Set once ethosu_init has succeeded. */
static int EthosReady;
...
/*
  Read back the NPU hardware information discovered during ethos_setup().

  The driver instance is private to this file, so the application obtains the
  hardware identity through here. Returns 0 on success, -1 if the NPU was never
  successfully initialized.
*/
int ethos_get_info (struct ethosu_hw_info *info) {

  if ((info == NULL) || (EthosReady == 0)) {
    return -1;
  }

  ethosu_get_hw_info(&EthosDriver, info);
